...
@unittest.skipUnless(FLASK_AVAILABLE, "Flask is not installed; install requirements.txt to run deployment tests")
class DeploymentAndSecurityTests(unittest.TestCase):
    def create_mode_app(self, deployment: str):
        temp = tempfile.TemporaryDirectory()
        self.addCleanup(temp.cleanup)
        with patch.dict(os.environ, {"CANDYTEST_DEPLOYMENT": deployment}, clear=True):
            from candytest.app import create_app
            app = create_app(Path(temp.name))
        app.testing = True
        return app

    def test_deployment_and_host_validation_keep_local_private_by_default(self):
        from candytest.app import configured_deployment, configured_host

        with patch.dict(os.environ, {}, clear=True):
            self.assertEqual(configured_deployment(), "local")
            self.assertEqual(configured_host(), "127.0.0.1")
        with patch.dict(os.environ, {"CANDYTEST_DEPLOYMENT": "local", "CANDYTEST_HOST": "::1"}, clear=True):
            self.assertEqual(configured_host(), "::1")
        with patch.dict(os.environ, {"CANDYTEST_DEPLOYMENT": "local", "CANDYTEST_HOST": "0.0.0.0"}, clear=True):
            with self.assertRaisesRegex(RuntimeError, "local 模式下.*回环"):
                configured_host()
        with patch.dict(os.environ, {"CANDYTEST_DEPLOYMENT": "production"}, clear=True):
            with self.assertRaisesRegex(RuntimeError, "只能是 local 或 server"):
                configured_deployment()

    def test_server_host_validation_allows_unspecified_private_and_global(self):
        from candytest.app import configured_host

        with patch.dict(os.environ, {"CANDYTEST_DEPLOYMENT": "server"}, clear=True):
            self.assertEqual(configured_host(), "0.0.0.0")
        for host in ("0.0.0.0", "::", "127.0.0.1", "::1", "10.0.0.2", "8.8.8.8"):
            with self.subTest(host=host), patch.dict(
                os.environ, {"CANDYTEST_DEPLOYMENT": "server", "CANDYTEST_HOST": host}, clear=True
            ):
                self.assertEqual(configured_host(), host)
        for host in ("224.0.0.1", "240.0.0.1"):
            with self.subTest(rejected_host=host), patch.dict(
                os.environ, {"CANDYTEST_DEPLOYMENT": "server", "CANDYTEST_HOST": host}, clear=True
            ):
                with self.assertRaisesRegex(RuntimeError, "组播或保留"):
                    configured_host()
        with patch.dict(os.environ, {"CANDYTEST_DEPLOYMENT": "server", "CANDYTEST_HOST": "not-an-ip"}, clear=True):
            with self.assertRaisesRegex(RuntimeError, "合法的 IP"):
                configured_host()

    def test_health_runtime_security_headers_and_no_cors(self):
        app = self.create_mode_app("local")
        ...
        for path in ("/", "/healthz", "/api/runtime"):
        ...
        self.assertEqual(client.get("/api/runtime").headers.get("Cache-Control"), "no-store")
        self.assertEqual(client.get("/healthz").headers.get("Cache-Control"), "no-store")
        self.assertEqual(client.get("/api/runtime").get_json()["deployment"], "local")

    def test_template_copy_reflects_deployment_mode(self):
        local_page = self.create_mode_app("local").test_client().get("/").get_data(as_text=True)
        server_page = self.create_mode_app("server").test_client().get("/").get_data(as_text=True)
        self.assertIn("本地明文", local_page)
        self.assertIn("回环地址", local_page)
        self.assertIn("服务器数据库明文", server_page)
        self.assertIn("必须通过 HTTPS 认证反代访问", server_page)
        self.assertNotIn("本地明文", server_page)

    def test_run_opens_browser_only_for_local_mode(self):
        import run

        with patch("run.configured_deployment", return_value="server"), \
             patch("run.configured_host", return_value="0.0.0.0"), \
             patch("run.configured_port", return_value=8765), \
             patch("run.create_app", return_value=object()), \
             patch("run.serve") as serve, \
             patch("run.webbrowser.open") as browser_open, \
             patch("run.threading.Timer") as timer:
            run.main()
        browser_open.assert_not_called()
        timer.assert_not_called()
        self.assertEqual(serve.call_args.kwargs["threads"], 8)

        with patch.dict(os.environ, {"CANDYTEST_OPEN_BROWSER": "0"}, clear=True), \
             patch("run.configured_deployment", return_value="local"), \
             patch("run.configured_host", return_value="127.0.0.1"), \
             patch("run.configured_port", return_value=8765), \
             patch("run.create_app", return_value=object()), \
             patch("run.serve"), \
             patch("run.threading.Timer") as timer:
