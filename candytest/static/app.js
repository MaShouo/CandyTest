(() => {
  ...
  const questionDefaults = { candy: { rounds: 5, effort: "low" }, cup: { rounds: 2, effort: "medium" }, probability: { rounds: 5, effort: "medium" }, dag10: { rounds: 5, effort: "medium" }, thibault_sottiaux: { rounds: 3, effort: "low", model: "gpt-6-astra" }, booker_2025: { rounds: 3, effort: "low", model: "gpt-6-astra" }, nobel_physics_2025: { rounds: 3, effort: "low", model: "gpt-6-astra" }, venice_golden_lion_2025: { rounds: 3, effort: "low", model: "gpt-6-astra" } };
  ...
  function setQuestionDefaults() { const defaults = questionDefaults[$("#question").value]; $("#modelOverride").value = defaults.model || ""; $("#rounds").value = defaults.rounds; $("#randomCandyFormat").style.display = $("#question").value === "candy" ? "" : "none"; setEfforts(defaults.effort); updateTestEstimate(); }
  ...
  async function init() { try { state.runtime = await api("/api/runtime"); renderEngines(); setQuestionDefaults(); await Promise.all([loadWebdav(), loadHistory(), loadGateways(), refreshCurrent()]); setInterval(() => refreshCurrent().catch(e => flash(e.message, true)), 2000); } catch (e) { flash(e.message, true); } }
  init();
})();
